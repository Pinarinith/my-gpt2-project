...
class GPT2SentimentClassifier(torch.nn.Module):
  ...
  def __init__(self, config):
    ...
    # 分类层（GPT-2 hidden_size -> num_labels）
  ...
  def forward(self, input_ids, attention_mask):
    ...
    # 获取 GPT-2 的隐藏状态 [batch_size, seq_len, hidden_size]
...
class SentimentDataset(Dataset):
  def __init__(self, dataset, args):
    ...
    self.p = args
    self.tokenizer = GPT2Tokenizer.from_pretrained('./local_gpt2_tokenizer')
...
def train(args):
  ...
  config = {
        'model_size': 'gpt2',           # 新增
        'd': 768,                       # GPT-2 small的hidden_size
        'l': 12,                        # 12层Transformer
        'num_heads': 12,                # 注意力头数
        'hidden_size': 768,             # 与d一致
