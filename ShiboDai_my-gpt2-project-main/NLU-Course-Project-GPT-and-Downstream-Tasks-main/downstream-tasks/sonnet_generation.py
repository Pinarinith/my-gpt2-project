...
class SonnetGPT(nn.Module):
  ...
  def __init__(self, args):
    ...
    # Add linear layer for token prediction
  ...
  def forward(self, input_ids, attention_mask):
    ...
    """
    ### YOUR CODE HERE
    # Get hidden states from GPT-2 [batch_size, seq_len, hidden_size]
...
def train(args):
  ...
  # 添加验证数据加载器
  held_out_dataloader = DataLoader(held_out_sonnet_dataset, batch_size=args.batch_size,
                                  collate_fn=sonnet_dataset.collate_fn)
  ...
  for epoch in range(args.epochs):
    ...
    if num_val_batches > 0:
      val_loss_avg = val_loss / num_val_batches
      ...
      if val_loss_avg < best_val_loss:
        ...
        no_improve = 0
        save_model(model, optimizer, args, f'best_{args.filepath}')  # 保存最佳模型
      else:
        no_improve += 1
      ...
      print("No validation data to compute loss.")
